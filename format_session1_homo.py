...
def read_node_atts(node_file, pyg_file, label_file=None):
    ...
    if osp.exists(pyg_file + ".nodes_ycc.pyg") == False:
        ...
        with open(node_file, "r") as rf:
            while True:
                ...
                node_id = int(info[0])  # node id
                node_type = "item"  # node type

                node_maps.setdefault(
                    node_type, {}
                )  # {"b":{"123":0,"213":1,"321":2},"item":{....}}
                node_id_v2 = len(node_maps[node_type])
                node_maps[node_type][node_id] = node_id_v2

                node_embeds.setdefault(
                    node_type, {}
                )  # {"b":{"1":np.array(),"2":np.array()},"item"....}
                lack_num.setdefault(node_type, 0)  # {"b":{""}}}
                if len(info[2]) < 50:
        ...
        nodes_dict["labels"] = {}  # {"labels":{"item":[labels]},"maps"}
        nodes_dict["labels"]["item"] = labels
        print("\n")
        print("Start saving pkl-style node information")
        pkl.dump(
            nodes_dict, open(pyg_file + ".nodes_ycc.pyg", "wb"), pkl.HIGHEST_PROTOCOL
        )
        print("Complete saving pkl-style node information\n")

    else:
        nodes = pkl.load(open(pyg_file + ".nodes_ycc.pyg", "rb"))
    ...
    if label_file is not None:
        ...
        train_idx = indices[train_val_random][: int(indices.shape[0] * 0.05)]
        val_idx = indices[train_val_random][int(indices.shape[0] * 0.99) :]
...
def format_pyg_graph(edge_file, node_file, pyg_file, label_file=None):
    if osp.exists(pyg_file + "__ycc.pt") and args.reload == False:
        ...
        return 0
    elif osp.exists(pyg_file + "ycc.pt"):
        print("skip")
        graph = torch.load(pyg_file + "ycc.pt")
    else:
        print("##########################################")
        print(
    ...
    with open(edge_file, "r") as rf:
        while True:
            ...
            source_type, dest_type = "item", "item"
    ...
    for edge_type in edges:
        source_type = "item"  # edges[edge_type]["source_type"]
        dest_type = "item"  # edges[edge_type]["dest_type"]
    ...
    print("Start saving into pyg data")
    torch.save(graph, pyg_file + "__ycc.pt")
    print("Complete saving into pyg data\n")
