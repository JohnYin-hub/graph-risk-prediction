...
def read_node_atts(node_file, pyg_file, label_file=None):
    ...
    if osp.exists(pyg_file + ".nodes2_ycc.pyg") == False:
        ...
        pkl.dump(
            nodes_dict, open(pyg_file + ".nodes2_ycc.pyg", "wb"), pkl.HIGHEST_PROTOCOL
        )
        print("Complete saving pkl-style node information\n")

    else:
        nodes = pkl.load(open(pyg_file + ".nodes2_ycc.pyg", "rb"))
    ...
    if label_file is not None:
        ...
        train_idx = indices[train_val_random][: int(indices.shape[0] * 0.5)]
        val_idx = indices[train_val_random][int(indices.shape[0] * 0.5) :]
